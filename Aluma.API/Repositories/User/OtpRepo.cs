...
namespace Aluma.API.Repositories
{
    public interface IOtpRepo : IRepoBase<OtpModel>
    {
        ...
        string SendConsentOTP(UserDto user, OtpTypesEnum otpType, int advisorId, int applicationId = 0);

        string ResendConsentOTP(UserDto user, int advisorId);
    ...
    public class OtpRepo : RepoBase<OtpModel>, IOtpRepo
    {
        ...
        public string SendOTP(UserDto user, OtpTypesEnum otpType, int applicationId = 0)
        {
            ...
            try
            {
                ...
                string otpMessage = otpType == OtpTypesEnum.Login ? "Aluma Capital: Herewith your OTP for signing in - " + newOtpNumber
                    ...
                    //: otpType == OtpTypesEnum.Consent ? "Aluma Capital: Dear " + client + ", herewith your OTP - " + newOtpNumber + " to approve consent for " + advisor + " to obtain your portfolio details from Astute." 
        ...
        public string SendConsentOTP(UserDto user, OtpTypesEnum otpType, int advisorId, int applicationId = 0)
        {
            SmsService.SmsRepo smsService = new();
            string result = string.Empty;
            try
            {
                ...
                AdvisorModel advisor = _context.Advisors.Where(a => a.Id == advisorId).First();
                UserModel advisorDetails = _context.Users.Where(a => a.Id == advisor.UserId).First();
                string advisorName = advisorDetails.FirstName + " " + advisorDetails.LastName;
                string client = user.FirstName + " " + user.LastName;
                string newOtpNumber = smsService.CreateOtp();
                string otpMessage = "Aluma Capital: Dear " + client + ", herewith your OTP - " + newOtpNumber + " to approve consent for " + advisorName + " to obtain your portfolio details from Astute.";
        ...
        public string ResendOTP(UserDto user)
        {
            ...
            try
            {
                ...
                string otpMessage = userOtp.OtpType == OtpTypesEnum.Login ? "Aluma Capital: Herewith your OTP for signing in - " + userOtp.Otp
                     ...
                     //: userOtp.OtpType == OtpTypesEnum.Consent ? "Aluma Capital: Dear "+ client + ", herewith your OTP - " + userOtp.Otp + " to approve consent for " + advisor + " to obtain your portfolio details from Astute."
        ...
        public string ResendConsentOTP(UserDto user, int advisorId)
        {
            SmsService.SmsRepo smsService = new();
            string result = string.Empty;
            try
            {                
                ...
                string advisorName = advisorDetails.FirstName + " " + advisorDetails.LastName;
                string client = user.FirstName + " " + user.LastName;
                string otpMessage = "Aluma Capital: Dear " + client + ", herewith your OTP - " + userOtp.Otp + " to approve consent for " + advisorName + " to obtain your portfolio details from Astute.";
